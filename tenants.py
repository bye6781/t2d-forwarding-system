...
async def invite_member(req: InviteMemberRequest, user=Depends(get_current_user)):
    ...
    # 检查成员数限制（使用 quota_service 统一检查，付费版自动跳过）
    can_add = await quota_service.check_resource_limit(user["tenant_id"], "member",
        len(await database.get_users_by_tenant(user["tenant_id"])))
    if not can_add:
        from app.core.config import settings
        plan = await database.fetchval("SELECT plan FROM tenants WHERE id = $1", user["tenant_id"]) or "free"
        limits = settings.PLAN_LIMITS.get(plan, settings.PLAN_LIMITS["free"])
        raise HTTPException(status_code=400, detail=f"成员数已达上限（{limits['member_limit']}人），请升级套餐")
...
async def change_plan(req: ChangePlanRequest, user=Depends(get_current_user)):
    ...
    tenant_id = user["tenant_id"]
    # 更新套餐
    await database.update_tenant_plan(tenant_id, req.plan)
    # 更新订阅配额
    limits = settings.PLAN_LIMITS[req.plan]
    await database.pool.execute(
        ...
        req.plan, limits["message_quota"], limits["tg_account_limit"], limits["mapping_limit"], tenant_id
    )
    return {"message": "套餐已变更", "limits": limits}
...
async def register_tenant(req: RegisterRequest):
    ...
    # 检查用户名唯一（全局）
    dup = await database.fetchrow("SELECT id FROM users WHERE username = $1", req.username)
    if dup:
        raise HTTPException(status_code=400, detail="用户名已存在")
    ...
    await database.create_subscription(
        tenant_id=tenant_id,
        plan="free",
        message_quota=free_limits.get("message_quota", 100),
